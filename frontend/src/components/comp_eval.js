...
class EvalComp extends Component {
  constructor(props) {
    ...
    this.state = {
      evals: -1,
      listOfMenuItems: [],
      listOfGraphTypes: ["bar", "line", "box", "kde", "area"],
      selectedColumns: "",
      selectedGraphType: "",
      outputHtml: "",
      outputChart: "",
  ...
  getElementSubstring = (elementName, extra_param) => {
    ...
    if (elementName == "input") {
      switch (extra_param) {
        case "checkbox":
          let tagStart = this.state.outputElementString.indexOf(
            "<" + elementName
          );
          let tagEnd = this.state.outputElementString.indexOf("<br>") + 4;
          if (tagStart == -1 || tagEnd == -1) return null;
          output = this.state.outputElementString.substr(
            tagStart,
            tagEnd - tagStart
          );
          let newText =
            this.state.outputElementString.slice(0, tagStart) +
      ...
      let tagEnd = this.state.outputElementString.indexOf(
        "</" + elementName + ">"
      );
      if (tagStart == -1 || tagEnd == -1) return null;
      output = this.state.outputElementString.substr(
        tagStart,
        tagEnd - tagStart + this.state.scriptWordLength + 1
      );

      let newText =
        this.state.outputElementString.slice(0, tagStart) +
        this.state.outputElementString.slice(
          tagEnd + this.state.scriptWordLength + 1
        );
    ...
    parse(inputScriptTag, {
      replace: (node) => {
        ...
        if (node.type === "script") {
          ...
          button.setAttribute(
            "class",
            node.attribs.class + " col-sm" + " Margin10Px"
          );
          ...
          switch (node.attribs.type) {
            case "checkbox":
              ...
              checkbox.setAttribute(
                "class",
                "form-check-input col-sm Margin10Px"
              );
  ...
  setOutputView = (value) => {
    ...
    while (
      numberOfParsedElements < this.state.maximumNumberOfControls &&
      this.state.outputElementString != "" &&
      this.state.outputElementString != null &&
  ...
  refereshValues = () => {
    this.state.refreshHandler();
    this.state.outputHtml = "";
    this.state.outputChart = "";
  };
      ...
      <div style={{ textAlign: "left", margin: "" }}>
        <div>
          <div class="row">
            ...
            <div class="col-6">          
                <FormControl fullWidth>
                  <InputLabel id="graph-simple-select-label">
                    Graph Type
                  </InputLabel>
                  <Select
                    labelId="graph-simple-select-label"
                    id="graph-simple-select"
                    value={this.state.selectedGraphType}
                    label="Graph Type"
                    onChange={this.handleGraphType}
                  >
                    {this.state.listOfGraphTypes.map((graph) => (
        ...
        <div>
          <Button
            ...
            onClick={this.GetColumnsAndComponents}
          >
            Load Selected Data
          </Button>
          <Button
            ...
            color="warning"
            className="Margin15Px"
            onClick={this.refereshValues}
          >
            Refresh Selections and back to top
          </Button>
          <Button
            variant="contained"
            className="InvisibleItem Margin15Px"
            data-name="RunCodeClick"
            onClick={this.callEvaluationServerWithQuery}
          >
            Run Code WithQuery
          </Button>
        </div>
        <div
          name="outputControls"
          className="form-group rounded Margin15Px"
          data-name="outputControls"
        ></div>
        <div name="outputHtml" data-name="outputHtml">
          {parse(this.state.outputHtml)}
        </div>
